...
def index(request):
    if request.user.is_authenticated():
        if request.user.user_profile.is_active:
            ...
            context = {"cart": "active", "forms": forms, "pwd_forms": pwd_forms, "username": request.user.username}
...
def source(request):
    ...
    sql = 'select a.id,a.asin,a.total,b.name as tag_name,a.createtime,a.starttime,a.sync_flag,CASE  WHEN a.sync_flag = 0 THEN "未分配"  ELSE "已分配" END AS sync_status from cart a left join tag b on a.tag_id = b.id where a.delflag=0 and a.createuser_id=%s' % request.user.id
...
def delete(request):
    ...
    try:
        if request.POST.get('id'):
            obj = Cart.objects.get(id=request.POST.get('id'))
            if obj.sync_flag == 1:
