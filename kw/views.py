...
def index(request):
    if request.user.is_authenticated():
        if request.user.user_profile.is_active:
            ...
            template = loader.get_template('kw.html')
            context = {"kw": "active", "forms": forms, "pwd_forms": pwd_forms, "username": request.user.username,"tags":tags}
...
def source(request):
    ...
    sql = 'select a.id,a.keyword,a.asin,a.total,b.name as tag_name,a.createtime,a.starttime,a.sync_flag,CASE  WHEN a.sync_flag = 0 THEN "未分配"  ELSE "已分配" END AS sync_status from kw a left join tag b on a.tag_id = b.id where a.delflag=0 and a.createuser_id=%s' % request.user.id
...
def delete(request):
    ...
    try:
        if request.POST.get('id'):
            obj = Kw.objects.get(id=request.POST.get('id'))
            if obj.sync_flag == 1:
...
def upload(request):
    ...
    cursor = connection.cursor()
    result = {}
    if 'keyword,asin,starttime,total,platform' == ','.join(title).strip().strip(","):
        items = []
        for line in datas:
            ...
            with transaction.atomic():
                for item in items:
                    if item:
                        ...
                        params = []
                        sql = "insert into kw(keyword,asin,total,createtime,starttime,delflag,finish,sync_flag,createuser_id,tag_id) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
                        params.append(keyword)
                        params.append(asin)
                        params.append(total)
                        params.append(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
...
def insert_log(request):
    ...
    sql = "insert into ab3_log(ab2_id,createtime,buyer_id,status,msg,ip) values (%s,%s,%s,%s,%s,%s)"
    params = []
    cursor = connection.cursor()
    params.append(task_id)
