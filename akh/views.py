...
def index(request):
    if request.user.is_authenticated():
        if request.user.user_profile.is_active:
            ...
            template = loader.get_template('akh.html')
            context = {"akh": "active", "forms": forms, "pwd_forms": pwd_forms, "username": request.user.username}
...
def source(request):
    ...
    sql = 'select a.id,a.asin,a.total,b.name as tag_name,a.createtime,a.starttime,a.sync_flag,CASE  WHEN a.sync_flag = 0 THEN "未分配"  ELSE "已分配" END AS sync_status from akh a left join tag b on a.tag_id = b.id where a.delflag=0 and a.createuser_id=%s' % request.user.id
...
def save(request):
    ...
    try:
        if sid:
            obj = Akh.objects.get(id=sid)
            if obj.sync_flag == 1:
                result = {"ret": 10000, "status": "failed", "msg": "任务已分配,无法进行修改!"}
                return HttpResponse(json.dumps(result), content_type="application/json")
    except Akh.DoesNotExist:
        pass
    if obj:
        form = AkhForm(request.POST or None, instance=obj)
    else:
        ...
        if Profile.objects.get(user_id=request.user.id).integral < total_integral:
            ...
            return HttpResponse(json.dumps(result), content_type="application/json")
        form = AkhForm(request.POST or None)
    if form.is_valid():
...
def delete(request):
    ...
    try:
        if request.POST.get('id'):
            obj = Akh.objects.get(id=request.POST.get('id'))
            if obj.sync_flag == 1:
